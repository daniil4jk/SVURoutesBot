...
class GetSomethingEvent extends ExpectedEvent<Message> {
...
class GetNumberEvent extends GetSomethingEvent {
...
class GetMonthInput extends GetSomethingEvent {
...
class ConfirmEvent extends ExpectedEvent<CallbackQuery> {
    private static final String[] monthNames =
    ...
    public ConfirmEvent(AtomicLong chatId, SimpleExecuter executer,
        ...
        super(q -> {
            if (String.valueOf(true).equals(q.getData())) {
                var newEntity = service.createNew(route.getId(), calendar.getTime());
